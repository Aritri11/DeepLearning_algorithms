...
class Optimizer:
    def __init__(self, params, lr=0.01):
        self.params = list(params)   # convert generator of params to list
        self.lr = lr                 # global learning rate
...
class SGDMomentum(Optimizer):
    def __init__(self, params, lr=0.01, momentum=0.9):
        ...
        # Keep track of velocity for each parameter (same shape as param)
...
class AdaGrad(Optimizer):
    def __init__(self, params, lr=0.01, eps=1e-8):
        super().__init__(params, lr)
        self.eps = eps
        # Accumulate sum of squares of gradients
...
class RMSProp(Optimizer):
    ...
    def step(self):
        """
        RMSProp update:
        E[g^2] <- beta * E[g^2] + (1 - beta) * grad^2
        w <- w - lr * grad / sqrt(E[g^2] + eps)
        """
        for i, p in enumerate(self.params):
            if p.grad is None:
                continue
...
# Dummy dataset & model
X = torch.randn(30, 10)
y = torch.randn(30, 1)    # regression targets
